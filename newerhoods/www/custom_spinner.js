/* When receiving message turn spinner on */
Shiny.addCustomMessageHandler('spinner-on', function(message) {
        $("#map").siblings(".load-container, .shiny-spinner-placeholder").show();
        $("#map").siblings(".load-container").siblings('.shiny-bound-output, .shiny-output-error').css('visibility', 'hidden');
        // if there is a proxy div, hide the previous output
       $("#map").siblings(".shiny-spinner-placeholder").siblings('.shiny-bound-output, .shiny-output-error').addClass('shiny-spinner-hidden');
      });

/* When receiving message turn spinner off */
Shiny.addCustomMessageHandler('spinner-off', function(message) {
        $("#map").siblings(".load-container, .shiny-spinner-placeholder").hide();
        $("#map").siblings(".load-container").siblings('.shiny-bound-output').css('visibility', 'visible');
        // if there is a proxy div, hide the previous output
       $("#map").siblings(".shiny-spinner-placeholder").siblings('.shiny-bound-output').removeClass('shiny-spinner-hidden');
      });
