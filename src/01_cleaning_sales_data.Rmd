...
for(i in c(1:dim(boro_year)[1])){
  ...
  tryCatch(
    {
      tmp <- read_excel(paste0("../data/annualized_sales/",boro_year$file[i]),skip=4)
    ...
    error=function(cond){
      error_cases <<- append(error_cases,paste0("../data/annualized_sales/",boro_year$file[i]))
      print(paste0("../data/annualized_sales/",boro_year$file[i]))
    }
  )
}
...
```{r}
assessment_tc1 <- mdb.get("../data/assessment/tc1.mdb",tables="tc1")
assessment_tc234 <- mdb.get("../data/assessment/TC234.mdb",tables="tc234")
